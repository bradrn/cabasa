...
class Monad m => AddHandler n m | m -> n where
    ...
    widgetAddEvents :: GI.Gtk.IsWidget o => Lens' (T.Application n) o -> [EventMask] -> m ()
    
instance AddHandler n (App n) where
...
addHandlers :: (KnownNat n, AddHandler n m) => m ()
addHandlers = do
    addMenuHandlers
    addCanvasHandlers
...
addControlButtonsHandlers :: (KnownNat n, AddHandler n m) => m ()
